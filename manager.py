...
class DSSManager:
        ...
        try:
            while self.running:
                try:
                    ...
                    # We still respond to the sender, even for shutdown.
                    self.socket.sendto(response.encode('utf-8'), addr)
                except Exception as e:
                    # Keep the server alive unless we're shutting down
                    if self.running:
                        print(f"Error: {e}")
        finally:
            try:
    ...
    def process_message(self, message):
        ...
        elif command in ("terminate-manager", "shutdown-manager", "shutdown"):  # <-- NEW
            return self.terminate_manager(parts[1:])
        else:
    ...
    def register_disk(self, params):
        ...
        # Checking if the disk already exists
    ...
    # ---------- NEW ----------
    def terminate_manager(self, params):
        """Handle terminate-manager/shutdown command"""
        # If you want a token, you could check it here via params.
        print("Shutdown requested; terminating after reply.")
        self.running = False
        return "SUCCESS|Manager terminating"
    # -------------------------
